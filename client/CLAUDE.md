...
|----------|---------|---------|
| UI Components | shadcn/ui | `npx shadcn@latest init` → `npx shadcn@latest add [name]` |
| Icons | lucide-react | `npm i lucide-react` |
| Forms | React Hook Form + zod | `npm i react-hook-form @hookform/resolvers zod` |
| Server State | TanStack Query v5 | `npm i @tanstack/react-query` |
| Client State | Zustand | `npm i zustand` |
| Toast | Sonner | `npm i sonner` |
| Animation | Motion | `npm i motion` |
| Dates | date-fns | `npm i date-fns` |
| API Mock | MSW | `npm i -D msw` |

Chỉ install khi CẦN. Không install trước hết.
